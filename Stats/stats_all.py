...
path_output= "/rds/general/user/meb22/home/UKBB/Stats/Results"
...
def compare_protein_expression(
    ...
    def preprocess_data(disease_df_, control_df, covariates_df):
    ...
    def process_matched_proteins(df_wide, matched_pairs_df):
        ...
        df_wide_cleaned = propagate_missing(df_wide, matched_pairs_df)
    ...
    def save_results_and_plot(results_df):
...
hc_df = pd.read_csv("/rds/general/user/meb22/home/UKBB/Prediction_proteomics/New/Training_healthycontrol.csv")
hc_df = hc_df[hc_df["Diagnosis"] == 0 ]
pd_df = pd.read_csv(f"{diagnoses_path}/Parkinson.csv")
pd_eids = pd.read_csv("/rds/general/user/meb22/home/UKBB/Prediction_proteomics/New/PD_filtered.csv")["eid"].unique().tolist()                    
neurodegenerative_training = pd.read_csv("/rds/general/user/meb22/home/UKBB/Prediction_proteomics/New/Neurodegenerative_training_date_diagnosis.csv")
neurodegenerative_training = neurodegenerative_training[["eid","date_diagnosis"]]
training_list = pd.read_csv("/rds/general/user/meb22/home/UKBB/Prediction_proteomics/New/Training_all.csv")["eid"].tolist()
pd_training_list = list(set(training_list) & set(pd_eids))
pd_df = pd_df[pd_df["eid"].isin(pd_training_list)]
...
filtered_merged.drop(columns=["protein_id"],inplace=True)
protein_cols = np.loadtxt("/rds/general/user/meb22/home/UKBB/Prediction_proteomics/New/columns_30pd.txt", dtype=str).tolist()

df_expr = filtered_merged[filtered_merged["ins_index"] == 0]
df_expr = df_expr[["eid","protein_abbr","Expression levels"]]
df_expr = df_expr[df_expr["protein_abbr"].isin(protein_cols)]


# Extract args
control_label = sys.argv[1]  
disease_label = sys.argv[2]  
diagnosis_window = sys.argv[3]  # e.g., "prodromals"

# Map to preloaded dataframes
label_to_df = {
    "PD": pd_df,
    "OND": neurodegenerative_training,
    "HC": hc_df[["eid"]]
}

# Run comparison
compare_protein_expression(
    label_to_df[disease_label],
    label_to_df[control_label],
    disease_label=disease_label,
    control_label=control_label,
    df_expr=df_expr,
    visit_df=visit_df,
    covariates_df=covariates_df,
    output_prefix=f"PDvs{control_label}_{diagnosis_window}",
