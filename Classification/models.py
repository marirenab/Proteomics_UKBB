...
def perform_kfold_evaluation(estimator, X, y, kf, params_space,metric_optimising):
    metrics_dict = {
...
df_metrics ,feature_importances= perform_kfold_evaluation(base_pipeline, X[protein_cols_list], y, kf,params_space,metric_optimising)
df_metrics.to_csv(f"/rds/general/user/meb22/home/UKBB/Prediction_proteomics/New/Classification/Results/Predictions{model_name}_{prefix}_{imputer_name}_{metric_optimising}_{dataset_name}_metrics.csv", index=False)
feature_importances.to_csv(f"/rds/general/user/meb22/home/UKBB/Prediction_proteomics/New/Classification/Results/Predictions{model_name}_{prefix}_{imputer_name}_{metric_optimising}_{dataset_name}_Featureimportance.csv", index=False)